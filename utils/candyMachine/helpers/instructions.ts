...
export async function createCandyMachineV2Account(anchorProgram: Program, candyData: CandyMachineData, payerWallet: PublicKey, candyAccount: PublicKey) {
  const size =
    CONFIG_ARRAY_START_V2 + 4 + candyData.itemsAvailable.toNumber() * CONFIG_LINE_SIZE_V2 + 8 + 2 * (Math.floor(candyData.itemsAvailable.toNumber() / 8) + 1);
